...
GAMMA = 0.99
LEARNING_RATE = 0.00025  # Further decreased for stability

EPSILON_START = 1.0
EPSILON_DECAY = 0.992  # Slightly adjusted for prolonged exploration
EPSILON_MIN = 0.01

TAU = 0.01  # Increased from 0.005 for faster target network updates
MAX_EPISODES = 1200  # Further increased to allow more training
...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        return (torch.FloatTensor(np.array([s[0] for s in transitions])),
                torch.LongTensor([s[1] for s in transitions]),
                torch.FloatTensor([s[2] for s in transitions]),
                torch.FloatTensor(np.array([s[3] for s in transitions])),
                torch.FloatTensor([s[4] for s in transitions]))
...
def main():
    ...
    for i in range(num_eval_episodes):
        ...
        state, _ = test_env.reset(seed=42 + i) # Use different seeds for each evaluation run
        done = False
        test_steps = 0
        ...
        while not done and test_steps < 500:
            ...
            test_steps += 1
        eval_rewards.append(test_steps)
        test_env.close()
        print(f"  Evaluation Episode {i+1}: Balanced for {test_steps} steps.")

    average_eval_reward = np.mean(eval_rewards)
    print(f"\n[Evaluation Complete] Average steps balanced over {num_eval_episodes} episodes: {average_eval_reward:.2f}!")
